...
new_points <- expand.grid(GDD = 100, 
...
new_data <- rbind(plot_data100, plot_data500, plot_data900) %>% 
  mutate(trt = fct_recode(trt,
                          "chlorimuron-ethyl" = "clas", 
                          "cloransulam-methyl" = "first", 
                          "imazethapyr" = "purs", 
                          "metribuzin" = "tric", 
                          "flumioxazin" = "spart",  
                          "saflufenacil" = "valor", 
...
ggplot(aes(x = trt, y = .pred, color = .pred)) + 
  ...
  coord_flip() + 
  theme_bw() +
  scale_y_continuous(limits = c(-2,112), breaks = c(0 ,25, 50, 75, 100)) +
  geom_errorbar(aes(ymin = .pred_lower, 
                    ymax = .pred_upper),
